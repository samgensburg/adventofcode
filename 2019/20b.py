...
visited = set()
while len(queue) > 0:
	(row, column), distance, level = queue.popleft()
	if (row, column, level) in visited:
		continue
	if level > 2000:
		continue

	visited.add((row, column, level))

	if (row, column) in portal_lookup:
		print(row, column, level)
		...
		if portal_name == 'ZZ':
			...
			is_outer = (row == 2 or column == 2 or row == height - 3 or column == width - 3)
			#print(row, column, is_outer)
			if not (is_outer and level == 0):
				...
				if is_outer:
					...
					new_level = level + 1
				queue.append((portal_loc, distance + 1, new_level))
	...
	for move_row, move_column in potential_moves:
		if grid[move_row][move_column] == '.':
			queue.append(((move_row, move_column), distance + 1, level))
